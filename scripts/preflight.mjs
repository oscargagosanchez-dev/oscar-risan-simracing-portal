...
for(const f of ['api/events.js','api/news.js','api/srp.js','api/youtube.js','api/health.js','srp.js','news-format.js','portal-next.js','donde-correr.js','simulador.js']){try{execFileSync(process.execPath,['--check',path.join(root,f)],{stdio:'pipe'})}catch(e){console.error('SYNTAX',f,String(e.stderr||e.message));failed=true}}
...
for(const [file,needles] of [['donde-correr.html',['id="fullEvents"','/donde-correr.js']],['simulador.html',['id="simEvents"','id="simNews"','/simulador.js']]]){const txt=fs.readFileSync(path.join(root,file),'utf8');for(const needle of needles){if(!txt.includes(needle)){console.error(file,'missing',needle);failed=true}}}
