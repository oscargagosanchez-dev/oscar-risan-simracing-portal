...
for(const needle of ['or-favorite-sims-v1','/buscar.html','favoritePanel','og:image','application/ld+json','/hoy.js','/semana.js','loadWeek']){if(!pnext.includes(needle)){console.error('Portal next missing',needle);failed=true}}
...
for(const needle of ['/api/news','/api/events','semanaSimracing','RESUMEN SEMANAL']){if(!week.includes(needle)){console.error('Weekly module missing',needle);failed=true}}
