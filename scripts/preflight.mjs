...
for(const f of ['api/events.js','api/news.js','api/srp.js','api/youtube.js','api/health.js','srp.js','news-format.js']){try{execFileSync(process.execPath,['--check',path.join(root,f)],{stdio:'pipe'})}catch(e){console.error('SYNTAX',f,String(e.stderr||e.message));failed=true}}
...
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');const app=fs.readFileSync(path.join(root,'app.js'),'utf8');const css=fs.readFileSync(path.join(root,'styles.css'),'utf8');const srp=fs.readFileSync(path.join(root,'srp.css'),'utf8');
...
for(const needle of ['/styles.css','/app.js','img-spa-lmp2-lmgt3.js','mobile-bottom','viewport-fit=cover','id="srp"','/srp.css','/srp.js']){if(!html.includes(needle)){console.error('HTML missing',needle);failed=true}}
