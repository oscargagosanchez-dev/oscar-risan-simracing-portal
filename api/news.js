...
function uniqueShort(xs,max=8){const out=[];for(const raw of xs||[]){const s=cleanHtml(raw).replace(/^[-–—•]+\s*/,'').trim();if(!s||s.length<4||s.length>120)continue;if(out.some(x=>x.toLowerCase()===s.toLowerCase()))continue;out.push(s);if(out.length>=max)break}return out}
function topicsFrom(html=''){
 const s=String(html||''),found=[];let m;
 const hrx=/<h[2-4][^>]*>([\s\S]*?)<\/h[2-4]>/gi;while((m=hrx.exec(s)))found.push(m[1]);
 const brx=/\[(?:h[1-6]|header|subheader)\]([\s\S]*?)\[\/(?:h[1-6]|header|subheader)\]/gi;while((m=brx.exec(s)))found.push(m[1]);
 return uniqueShort(found,8)
}
function videoEmbed(url=''){
 const u=decodeHtml(String(url||'').replace(/&amp;/g,'&')).trim();let m;
 if((m=u.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([A-Za-z0-9_-]{6,})/i)))return`https://www.youtube.com/embed/${m[1]}`;
 if((m=u.match(/vimeo\.com\/(?:video\/)?(\d+)/i)))return`https://player.vimeo.com/video/${m[1]}`;
 return''
}
function videosFrom(html=''){
 const s=String(html||''),urls=[];let m;
 const attr=/(?:src|href)=["']([^"']*(?:youtube\.com|youtu\.be|vimeo\.com)[^"']*)["']/gi;while((m=attr.exec(s)))urls.push(m[1]);
 const bare=/https?:\/\/[^\s"'<>]*(?:youtube\.com|youtu\.be|vimeo\.com)[^\s"'<>]*/gi;while((m=bare.exec(s)))urls.push(m[0]);
 return [...new Set(urls.map(videoEmbed).filter(Boolean))].slice(0,4)
}
...
async function translateItem(item){
 ...
 const [title,summary,...topics]=await Promise.all([translateText(originalTitle),translateText(originalSummary),...(item.topics||[]).map(translateText)]);
 return {...item,title,summary,topics:uniqueShort(topics,8),originalTitle,originalSummary,language:'es'};
}
...
async function steamNews(src){try{const u=`https://api.steampowered.com/ISteamNews/GetNewsForApp/v2/?appid=${src.appid}&count=20&maxlength=2600&format=json`;const r=await fetch(u,{headers:UA,signal:AbortSignal.timeout(7000)});if(!r.ok)throw new Error('steam');const j=await r.json();return (j?.appnews?.newsitems||[]).map(n=>({
 id:`steam-${src.appid}-${n.gid}`,
 category:src.category,
 title:cleanHtml(n.title),
 summary:cleanHtml(n.contents).slice(0,700),
 source:src.label,
 sourceUrl:n.url||`https://store.steampowered.com/news/app/${src.appid}`,
 image:imageFrom(n.contents),
 videos:videosFrom(n.contents),
 topics:topicsFrom(n.contents),
...
async function traxionNews(){
 try{
  ...
  return posts.map(p=>{
   ...
   const excerpt=cleanHtml(p?.excerpt?.rendered||raw).slice(0,700);
   ...
   const image=media?.source_url||media?.media_details?.sizes?.large?.source_url||imageFrom(raw);
   return {id:`traxion-${p.id}`,category,title,summary:excerpt,source:'Traxion',sourceUrl:p.link||'https://traxion.gg/category/news/',image,videos:videosFrom(raw),topics:topicsFrom(raw),date:p.date?new Date(p.date).toISOString():new Date().toISOString(),tags:tagsFor(category,title),sourcePriority:2};
  ...
  try{
   ...
   for(const block of blocks.slice(0,40)){
    ...
    const title=grab('title'),summary=grab('description').slice(0,700),link=grab('link'),category=categoryFromText(`${grab('category')} ${title} ${summary}`);if(!title)continue;
    const dateRaw=grab('pubDate');out.push({id:`traxion-feed-${Buffer.from(link||title).toString('base64url').slice(0,32)}`,category,title,summary,source:'Traxion',sourceUrl:link||'https://traxion.gg/category/news/',image:imageFrom(block),videos:videosFrom(block),topics:topicsFrom(block),date:dateRaw?new Date(dateRaw).toISOString():new Date().toISOString(),tags:tagsFor(category,title),sourcePriority:2});
   }
   return out;
  }catch{return []}
 }
...
async function overtakeNews(){
 try{
  ...
  while((m=rx.exec(html))&&out.length<40){
   ...
   const start=Math.max(0,m.index-900),end=Math.min(html.length,rx.lastIndex+1800),context=html.slice(start,end);const category=categoryFromText(context+' '+title);
   const dateMatch=context.match(/<time[^>]+datetime=["']([^"']+)["']/i);const metaDesc=context.match(/(?:article-body|message-body|contentRow-snippet|articlePreview-description)[^>]*>([\s\S]{20,1200}?)<\//i);let summary=cleanHtml(metaDesc?.[1]||'');if(!summary){const text=cleanHtml(context);const pos=text.toLowerCase().indexOf(title.toLowerCase());summary=(pos>=0?text.slice(pos+title.length):text).replace(/^(today|yesterday|monday|tuesday|wednesday|thursday|friday|saturday|sunday)[^A-Z]*/i,'').slice(0,700)}
   const image=imageFrom(context);seen.add(url);out.push({id:`overtake-${url.match(/\.(\d+)\/?$/)?.[1]||Buffer.from(url).toString('base64url').slice(0,24)}`,category,title,summary,source:'OverTake',sourceUrl:url,image,videos:videosFrom(context),topics:topicsFrom(context),date:dateMatch?new Date(dateMatch[1]).toISOString():new Date().toISOString(),tags:tagsFor(category,title),sourcePriority:2});
  }
  return out;
 }catch{return []}
}
...
const FALLBACK=[
 {id:'fallback-lmu-v14',category:'Le Mans Ultimate',title:'Le Mans Ultimate V1.4 y sus últimos hotfixes',summary:'Le Mans Ultimate ha recibido la actualización V1.4 y posteriores correcciones centradas en online, cambios de piloto, físicas, Race Watch y el calendario WEC 2026.',source:'Le Mans Ultimate / Steam',sourceUrl:'https://steamcommunity.com/app/2399420/announcements/',image:'',videos:[],topics:['Actualización V1.4','Correcciones posteriores'],date:'2026-07-30T10:00:00.000Z',tags:['LMU','ACTUALIZACIÓN'],language:'es'},
 {id:'fallback-acevo-08',category:'Assetto Corsa EVO',title:'Assetto Corsa EVO Early Access 0.8 disponible',summary:'Kunos continúa ampliando Assetto Corsa EVO con la versión Early Access 0.8 y nuevas mejoras para el simulador.',source:'Assetto Corsa',sourceUrl:'https://assettocorsa.gg/assetto-corsa-evo/',image:'',videos:[],topics:['Early Access 0.8'],date:'2026-07-08T10:00:00.000Z',tags:['AC EVO','ACTUALIZACIÓN'],language:'es'}
];
